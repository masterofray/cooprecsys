...
artifact registration for the LTR pipeline.

All run-level state is stored on ``self`` so the caller can inspect
``self.run_id`` after the context is closed.

Classes
-------
MLflowMonitor — thin, opinionated wrapper around the MLflow Python SDK.
"""
...
class MLflowMonitor:
    ...
    run_name:
        Optional descriptive run name.  Defaults to ``"ltr_run"``.

    Attributes (populated after context exit)
    -----------------------------------------
    run_id : str | None
    ...
    def __init__(
        ...
        self._run:    Optional[mlflow.ActiveRun] = None  # type: ignore[type-arg]
    ...
    def __exit__(self, exc_type: Any, exc_val: Any, exc_tb: Any) -> None:
        if exc_type is not None:
            logger.error(
                "MLflow run ending with exception: %s — %s",
                exc_type.__name__, exc_val,
            )
        mlflow.end_run()
        logger.info("MLflow run ended — run_id=%s", self.run_id)
    ...
    def log_metrics(self, metrics: Dict[str, float]) -> None:
        ...
        ----------
        metrics:
            Key-value metrics dict (values must be numeric).
        """
    ...
    def log_artifacts(self, paths: list[str]) -> None:
        ...
        ----------
        paths:
            List of absolute or relative filesystem paths.
        """
        for path in paths:
    ...
    def log_model(self, booster: lgb.Booster) -> None:
        ...
        ----------
        booster:
            Trained :class:`lgb.Booster` instance.
        """
    ...
    def log_tuner_summary(self, summary: Dict[str, Any]) -> None:
        ...
        ----------
        summary:
            Dict returned by :meth:`BayesianTuner.summary`.
        """
        if not summary:
    ...
    def log_all(
        ...
        params:
            Training hyper-parameter dict.
        metrics:
            Evaluation metrics dict.
        runtime_minutes:
            Wall-clock training duration.
        artifact_paths:
            File paths to log as artefacts (PNGs, HTML report, etc.).
        tuner_summary:
            Optional Bayesian tuner summary dict.
        """
        logger.info("MLflowMonitor.log_all() — logging full run.")
        ...
        logger.info("MLflowMonitor.log_all() — complete.")
